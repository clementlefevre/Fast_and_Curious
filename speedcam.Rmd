...
p1 <- ggplot(df %>% filter(as.Date(datetime)>=ymd('2018-04-08')),aes(week_day_hms,speed, color=type))+ geom_jitter(alpha=0.2)+geom_smooth()+theme(legend.position="top",legend.title=element_blank(),axis.title.x=element_blank())+scale_colour_manual(values=cbPalette)
...
groupy.per.date.stacked <- groupy.per.date %>% select(total.over.speed,total.bikes,TM,date) %>% gather(key,value,-date)

p <- ggplot(groupy.per.date.stacked,aes(date,value))+geom_line(aes(color=key))+scale_color_manual(values=c('#77003c','#c33333',"#9acd32"))+theme(legend.position="top",legend.title=element_blank(),axis.title.x=element_blank())
p <- p  + facet_wrap( ~ key, nrow = 3,scales='free_y') +ggtitle('Per day - April-Mai 2018')
p
...
```{r}
 ggplot(data=groupy, aes(fill=type)) + stat_identity(data = groupy %>% filter(datetime.index.hour>'2018-04-08'  & type=='car'), aes(datetime.index.hour, TN), geom = "bar", alpha = 0.8,position = "dodge")+theme(axis.text.x = element_text(angle=90))+
  scale_x_datetime(date_breaks = "1 day",labels = date_format("%a"))+theme(legend.position="top",legend.title=element_blank(),axis.title.x=element_blank())+scale_fill_manual(values='#77003c')+theme(legend.position="none")+ggtitle("Speed limit breaches per hour") + 
     theme(plot.title = element_text(lineheight=.8, face="bold"))
```
...
```
```{r}
model <- lm( total.over.speed ~ TM+total.bikes, data = groupy.per.date)
summary(model)
```
